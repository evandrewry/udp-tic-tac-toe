...
public enum ServerPacketType {
	ACK(AcknowledgementPacket.class), CURRENT_GAME_STATE(
			CurrentGameStatePacket.class), CURRENT_USERS_LIST(
			CurrentUsersListPacket.class), GAME_RESULT(GameResultPacket.class), ILLEGAL_MOVE(
			IllegalMovePacket.class), LOGIN_ACK(
			LoginAcknowledgementPacket.class), PLAY_REQUEST_ACK(
			PlayRequestAcknowledgementPacket.class), PLAY_REQUEST(
			PlayRequestPacket.class);
	...
	public ServerPacket fromPayload(Payload payload) {
		try {
			return (ServerPacket) clazz.getDeclaredConstructor(Payload.class)
					.newInstance(payload);
